...
static void changeByteEndian(unsigned char* data, int len) {
    for (int i = 0; i < len; i++) {
        data[i] = static_cast<unsigned char>(((data[i] & 0x0F) << 4) | ((data[i] >> 4) & 0x0F));
    }
}

//static g726_state_t g726_state = {0};

void initG726State(int bitCount, g726_state_t *g726_state)
{
    g726_init(g726_state, 8000 * bitCount);
}

int decodeG726(unsigned char* inData, int len, short* outData, g726_state_t *g726_state)
{
    changeByteEndian(inData, len);
    int bitCount = g726_state->bits_per_sample;
    switch(bitCount)
    {
        case 2:
            return g726_decode16(g726_state, outData, inData, len);
        case 3:
            return g726_decode24(g726_state, outData, inData, len);
        case 4:
            return g726_decode32(g726_state, outData, inData, len);
        case 5:
            return g726_decode40(g726_state, outData, inData, len);
        default:
            return 0;
    }
}

int encodeG726(short* inData, int len, unsigned char* outData, g726_state_t *g726_state)
{
    int bitCount = g726_state->bits_per_sample;
    switch(bitCount)
    {
        case 2:
            return g726_encode16(g726_state, outData, inData, len);
        case 3:
            return g726_encode24(g726_state, outData, inData, len);
        case 4:
            return g726_encode32(g726_state, outData, inData, len);
        case 5:
            return g726_encode40(g726_state, outData, inData, len);
        default:
            return 0;
    }
}
