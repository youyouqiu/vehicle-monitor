...
public class TransportLicenseCameraResultActivity extends Activity {
    ...
    protected void onCreate( Bundle savedInstanceState) {
        ...
        dialog = new LoadingDialog.Builder(TransportLicenseCameraResultActivity.this)
                .setMessage("识别中...")
                .setCancelable(false)
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                OCR.getInstance(TransportLicenseCameraResultActivity.this).initAccessToken(new OnResultListener<AccessToken>() {
                                    @Override
                                ...
                                // 调用通用文字识别服务
                                OCR.getInstance(TransportLicenseCameraResultActivity.this).recognizeGeneralBasic(param, new OnResultListener<GeneralResult>() {
                                    @Override
                                    public void onResult(GeneralResult result) {
                                        try {
                                            JSONObject jsonObject = JSONObject.parseObject(result.getJsonRes());
                                            String transportNumber = null;
                                            Intent intent = new Intent(TransportLicenseCameraResultActivity.this, ConfirmTransportLicenseActivity.class);
                                            JSONArray jsonArray = jsonObject.getJSONArray("words_result");
                                            for (Object object : jsonArray) {
                                                JSONObject jb = JSONObject.parseObject(object.toString());
                                                String re = jb.getString("words");
                                                if (re.contains("道路运输证号")) {
                                                    String[] res = re.split("道路运输证号");
                                                    transportNumber = res[res.length - 1];
                                                }
                                            }
                                            if (transportNumber == null) {
                                                throw new NullPointerException();
                                            }
                                            intent.putExtra("transportNumber", transportNumber);
