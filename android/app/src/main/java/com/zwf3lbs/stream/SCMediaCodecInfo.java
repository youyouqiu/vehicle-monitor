...
public class SCMediaCodecInfo {
	private static final String TAG = SCMediaCodecInfo.class.getSimpleName();
	...
	private static synchronized Map<String, Integer> getKnownCodecList() {
		...
		// Lenovo A788t
		sKnownCodecList
				.put("OMX.MARVELL.VIDEO.HW.CODA7542DECODER", RANK_TESTED);
	...
	public static SCMediaCodecInfo setupCandidate(MediaCodecInfo codecInfo,
                                                                       String mimeType) {
		if (codecInfo == null)
			return null;
		...
		SCMediaCodecInfo candidate = new SCMediaCodecInfo();
	...
	public boolean dumpProfileLevels(String mimeType) {
		try {
	...
	public boolean checkResolution(String mimeType, int width, int height) {
		...
		try {
			...
			boolean bret = videoCap.isSizeSupported(width, height);
			
			if(!bret) {
				Range<Integer> widthR = videoCap.getSupportedWidths();
				Range<Integer> heightR = videoCap.getSupportedHeights();
				Log.w(TAG,"width Range:"+widthR.toString()+"\nhegiht Range:"+heightR.toString());
			}
			
			return bret;
		} catch (Throwable e) {
			Log.w(TAG,"profile-level: exception");
			return false;
		}
	}
