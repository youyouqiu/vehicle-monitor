...
package com.zwf3lbs.map.clusterutil.ui;
...
public class IconGenerator {
    ...
    private LinearLayout mRotationLayout;
    ...
    public IconGenerator(Context context) {
        ...
        mRotationLayout = (LinearLayout) mContainer.getChildAt(0);
    ...
    public void setContentRotation(int degrees) {
        mRotationLayout.setVerticalGravity(degrees);
    }
    ...
    private static int getTextStyle(int style) {
        ...
        public BitmapDescriptor makeIcons(int num) {
            View inflate =LayoutInflater.from(mContext).inflate(R.layout.vehicle_cluster, null);
            TextView viewById1 = inflate.findViewById(R.id.onTextView);
            viewById1.setText(num+"");
            viewById1.getPaint().setFakeBoldText(true);
            float screenDensity = ScreenUtils.getScreenDensity();
//        layoutParams.width = 200;
//        layoutParams.height = 200;
//            viewById.setLayoutParams(layoutParams);
            Float wid = 26 * screenDensity;
            Float hig = 35 * screenDensity;
            Bitmap bitmap = ImageUtils.compressByScale(BitmapDescriptorFactory.fromView(inflate).getBitmap(), wid.intValue(), hig.intValue());
