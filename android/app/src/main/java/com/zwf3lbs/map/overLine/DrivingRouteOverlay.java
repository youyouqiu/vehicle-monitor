...
//                }
        ...
        if (mRouteLine.getStarting() != null) {
            overlayOptionses.add((new MarkerOptions())
                    .position(mRouteLine.getStarting().getLocation())
                            .icon(startMarker).zIndex(10).visible(false));
        }
        BitmapDescriptor terminalMarker = getTerminalMarker();
        if (mRouteLine.getTerminal() != null) {
            MarkerOptions markerOptions = (new MarkerOptions())
                    .position(mRouteLine.getTerminal().getLocation())
                    .icon(terminalMarker).zIndex(9);
            Bundle bundle = new Bundle();
            bundle.putString("last","true");
        ...
        if (mRouteLine.getAllStep() != null
            ...
            PolylineOptions option = new PolylineOptions().points(points).textureIndex(traffics)
                    .width(5).dottedLine(isDotLine).focus(true)
                        .color(0xff0000ff).zIndex(0);
            if (isDotLine) {
    ...
    public BitmapDescriptor getStartMarker() {
        return CommonUtil.createStartIco();
    }
    ...
    public BitmapDescriptor getTerminalMarker() {
         return CommonUtil.createEndIco();
    }
