...
class HandleTool extends Component {
  static propTypes = {
    ...
    hasInterval: PropTypes.bool.isRequired, // 是否真的在播放了，如果是，有interval
  }
  ...
  shouldComponentUpdate(nextProps, nextState) {
    const propsEqual = deepEqual(this.props, nextProps,
      ['playFlag', 'cameraFlag', 'mapShow', 'unique', 'playState', 'audioFlag', 'screenFlag']);
  ...
  // 拍照
  toggleCameraFun = () => {
    const { playState, hasInterval } = this.props;
    if (playState) {
      if (!hasInterval) {
        toastShow('视频正在请求中，稍后方可拍照', { duration: 2000 });
        return;
      }
