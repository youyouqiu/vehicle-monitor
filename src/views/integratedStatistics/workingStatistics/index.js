...
import { judgeUserIfOwnSend, getWorkingStatisticalMonitor } from '../../../server/getData';
...
class WorkingStatistics extends Component {
  ...
  getDefaultMonitorsFun = () => {
    ...
    if (checkMonitors.item && checkMonitors.item.goTarget === 'workingStatistics') {
      const judgeFun = judgeUserIfOwnSend;
      const getDefaultMonitorFun = getWorkingStatisticalMonitor;
      judgeFun().then((res) => {
...
export default connect(
  ...
  dispatch => ({
    ...
    reset: () => {
      dispatch({ type: 'workingStatistics/RESET_DATA' });
    },
  }),
)(WorkingStatistics);