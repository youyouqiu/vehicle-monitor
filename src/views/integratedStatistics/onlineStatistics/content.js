...
export default class OnlineStatisticsContent extends Component {
  data={
    startTime: null,
    oldStartTime: null,
  }
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    // const { barDetailData: prevbarDetailData } = this.props;
    ...
    //   const detailsObj = details.map(x => ({
    //     value: x.onlineFlag,
    //     name: x.time.substr(x.time.length - 2, 2),
    //     ifOnline: x.onlineFlag,
    //   }));
 ...
 getCheckMonitor=(num, startTimeStr, endTimeStr) => {
  ...
  handlePress=(index, sameDay) => {
    const { getDetailData, resetDetails, barChartData } = this.props;
    ...
    const barChartDataObj = barChartData.toJS();
  ...
  aggregateData(data) {
    ...
    for (let i = 0; i < data.length; i += 1) {
      const element = Number(data[i].ratio);

      sum += element;
      if (max === null || element >= max) {
        if (element > max) {
          maxCar = [data[i].carLicense];
        } else {
          maxCar.push(data[i].carLicense);
        }
        max = element;
      }
      if (min === null || element <= min) {
        if (element < min) {
          minCar = [data[i].carLicense];
        } else {
          minCar.push(data[i].carLicense);
        }
        min = element;
      }
    }

    const averageValue = data.length === 0 ? 0 : (sum / data.length).toFixed(1);
  ...
  render() {
    ...
    const barDetailsData = this.detailsDataAssembly(barDetailData);
          ...
          <View style={styles.block}>
            ...
            <View style={styles.totalContainer}>
              <View style={styles.totalWraper}>
                <Image source={IconOnline1} resizeMode="contain" style={styles.alarmIcon} />
                <Text style={[styles.totalText, Platform.OS !== 'ios' ? { fontFamily: '' } : null]}>
                  <Text>{`${getLocale('enrollmentRate')} `}</Text>
                  <Text style={[styles.totalNumber, Platform.OS !== 'ios' ? { fontFamily: '' } : null]}>{toFixed(agg.averageValue, true)}</Text>
                  <Text>{`${getLocale('percent')}`}</Text>
                </Text>
              </View>
            </View>
            <View style={styles.totalContainer}>
              <View style={[styles.maxMinContainer, styles.maxContainer]}>
                <Text style={[styles.maxMinText, styles.txtRight]}>
                  {`${getLocale('highest')} `}
                  <Text style={styles.maxMinNumber}>{toFixed(agg.max, 1, true)}</Text>
                  {`${getLocale('percent')} `}
                </Text>
                <Text style={[styles.maxMinText, styles.txtRight]}>
              ...
              <View style={styles.maxMinContainer}>
                <Text style={styles.maxMinText}>
                  {`${getLocale('lowest')} `}
                  <Text style={styles.maxMinNumber}>{toFixed(agg.min, true)}</Text>
                  {`${getLocale('percent')} `}
                </Text>
                <Text style={styles.maxMinText}>
