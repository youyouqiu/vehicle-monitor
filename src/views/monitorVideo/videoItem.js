...
class VideoItem extends Component {
      ...
      let message = {
        data: {
          ...
          msgBody: {
            ...
            channelNumber: JSON.stringify(item.logicChannel), // 通道号
            sampleRate: JSON.stringify(playMessage.samplingRateStr || 8000), // 采样率
            channelCount: JSON.stringify(playMessage.vocalTractStr || 1), // 声道数
            ...
            streamType: JSON.stringify(item.streamType), // 码流类型0：主码流，1：子码流
            deviceType: playMessage.deviceType, // 设备类型
          },
        },
        panoramic: JSON.stringify(item.panoramic), // 是否360全景
      };
      if (!ifOpenVideo) { // 停止
        message = {
          data: {
  ...
  msgChangeFun = ({ code, msg }) => {
    if (code < 0) {
      const { item } = this.props;
      if (item) {
        toastShow(`视频通道${item.physicsChannel}:${msg}`, { duration: 600 });
      }
    }
  }
  ...
  render () {
    const {
      item, brand, choosenVideo, screenFlag, playMessage, isStopVideo,
    } = this.props;
    const {
      ifOpenVideo,
      ifSuccess,
    ...
    let sampleRate;
    if (item) {
      const { audioSampling } = item;
      url = `ws://${realTimeVideoIp}:${realTimeVideoPort}/${playMessage && playMessage.simcardNumber}/${item.physicsChannel}/0`;
