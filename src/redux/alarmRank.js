...
// action
const INIT_ACTION = { type: 'alarmRank/SAGA/INIT_ACTION' };
const INIT_START_ACTION = { type: 'alarmRank/INIT_START_ACTION' };
const INIT_SUCCESS_ACTION = { type: 'alarmRank/INIT_SUCCESS_ACTION' };
const INIT_FAIL_ACTION = { type: 'alarmRank/INIT_FAIL_ACTION' };

const GET_DETAILS_START_ACTION = { type: 'alarmRank/GET_DETAILS_START_ACTION' };
const GET_DETAILS_ACTION = { type: 'alarmRank/SAGA/GET_DETAILS_ACTION' };
const GET_DETAILS_SUCCESS_ACTION = { type: 'alarmRank/GET_DETAILS_SUCCESS_ACTION' };
const GET_DETAILS_FAIL_ACTION = { type: 'alarmRank/GET_DETAILS_FAIL_ACTION' };

const RESET_DETAIL_ACTION = { type: 'alarmRank/SAGA/RESET_DETAIL_ACTION' };
const RESET_DETAIL_START_ACTION = { type: 'alarmRank/RESET_DETAIL_START_ACTION' };
const RESET_DETAIL_SUCCESS_ACTION = { type: 'alarmRank/RESET_DETAIL_SUCCESS_ACTION' };

const RESET_DATA = { type: 'alarmRank/RESET_DATA' };
...
const alarmRankReducers = (state = defaultState, { type, payload }) => {
  let newState = null;
  switch (type) {
    ...
    case INIT_SUCCESS_ACTION.type:
      newState = state.merge({
        ...
        pieChartData: payload.pieChartData,
        currentIndex: null,
...
function* initRequest ({ payload }) {
  const {
    monitors: vehicleIds, startTime, endTime,
  } = payload;
  yield put({
  ...
  if (payload.riskType) {
    param.riskType = payload.riskType;
  }


  const [barResult, pieResult] = yield all([
    call(getAlarmRankBar, param),
    call(getAlarmRankPie, { vehicleIds, startTime, endTime }),
  ]);

  if (barResult === false || barResult.statusCode !== 200
    || pieResult === false || pieResult.statusCode !== 200) {
