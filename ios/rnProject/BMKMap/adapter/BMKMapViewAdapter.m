...
+ (void)mapView:(BMKMapView *)mapView dampZoomingMapLevelFromCurrentValue:(float)currentLevel
 ToSettingValue:(float)settingLevel{
  float unitZoomLevelLastTime = (float) (MAP_ZOOM_DURATIONS / MAP_ZOOM_NUM);
  for (int i = 1; i<=MAP_ZOOM_NUM; i++) {
    float realTime = i * unitZoomLevelLastTime;
    /*方程y = aX^2 + bX + C */
    float a = (float) ((currentLevel - settingLevel) / (MAP_ZOOM_DURATIONS * MAP_ZOOM_DURATIONS));
    float b = (float) ((-2) * a * MAP_ZOOM_DURATIONS);
  ...
  for (UIView *subView in mapview.subviews) {
    //if ([subView isKindOfClass:[mapViewClass class]]) {
      ...
      for (UIView *subclassView in subView.subviews) {
         if ([NSStringFromClass([subclassView class]) isEqualToString:@"BMKTapDetectingView"]) {
...
- (void)hookMethodsInDetectingView:(UIView *)mapTapDetectingView{
  void (^MethodHookedBlock)(id<AspectInfo>) = ^(id<AspectInfo>hookedObject){
    ...
    if ([NSStringFromSelector(hookedSelector) isEqualToString:@"aspects__handleBMKDoubleBeginTouchPoint"]) {    //双指运动触摸屏幕
      [_mapAdpater hookedMethod_handleDoubleBeginTouchPoint];
    }else if ([NSStringFromSelector(hookedSelector) isEqualToString:@"aspects__handleBMKDoubleMoveTouchPoint"]) {//处理双指移动
      //TODO:
    }else if ([NSStringFromSelector(hookedSelector) isEqualToString:@"aspects__handleBMKDoubleEndTouchPoint"]){  //双指移动停止
      [_mapAdpater hookedMethod_handleDoubleEndTouchPoint];
      
    }else if ([NSStringFromSelector(hookedSelector) isEqualToString:@"aspects__handleBMKScale:"]){                //"handleScale: 手动改变地图的缩放等级的触发事件
  ...
  for (NSString * selString in [mapTapDetectingView.class arrayOfInstanceMethods]) {
    if (![selString isEqualToString:@"handleBMKDoubleBeginTouchPoint"] && ![selString isEqualToString:@"handleBMKDoubleEndTouchPoint"] && ![selString isEqualToString:@"handleBMKScale:"] && ![selString isEqualToString:@"handleBMKDoubleMoveTouchPoint"]) {
      continue;
    }
...
- (void)parabola_calculateOneCoefficient{
  // 放大地图系数为正，缩小为负
  _oneCoefficient = (float) (((_zoomLevelPinEnd - _zoomLevelPinStart) - _quadraticCoefficient * (_tPinMoving * _tPinMoving)) / (_tPinMoving));
}
...
- (void)parabola_refreshMapZoomLevel{
  ...
  float unitZoomLevelDuringTime = (float) ((_tStirless - _tPinMoving) / MAP_ZOOM_NUM);
  for (int i = 1; i <= MAP_ZOOM_NUM; i++) {
    float tempZoomLevel = (float) [self parabola_calculateMapZoomLevelAtRealTime:(float) (_tPinMoving + i * unitZoomLevelDuringTime)];
...
- (void)changeMapLevelByStep{
  ...
  float tempZoomLevel = (float) [self parabola_calculateMapZoomLevelAtRealTime:(float) (_tPinMoving + time)];
  [_mapView setZoomLevel:tempZoomLevel];
}
