...
static NSString *const URL_IOCR = @"https://aip.baidubce.com/rest/2.0/solution/v1/iocr/recognise";
static NSString *const URL_FORM = @"https://aip.baidubce.com/rest/2.0/ocr/v1/form";
static NSString *const URL_CAR = @"https://aip.baidubce.com/rest/2.0/ocr/v1/license_plate";
...
- (void) formRecognitionFromImage: (UIImage*)image
                   ...
                   successHandler: (void (^)(id result))successHandler
                      failHandler: (void (^)(NSError* err))failHandler; {
  NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:options];
  NSData *imageData = [self jpgDataWithImage:image sizeLimit:10240000];
  dict[@"image"] = [imageData base64EncodedStringWithOptions:0];
  
  [self _apiRequestWithURL:URL_FORM
                   options:dict
            successHandler:successHandler
               failHandler:failHandler];
}
...
- (void)detectVehicleLicenseFromImage:(UIImage *)image withOptions:(NSDictionary *)options successHandler:(void (^)(id result))successHandler failHandler:(void (^)(NSError *err))failHandler {
  ...
  NSData *imageData = [self jpgDataWithImage:image sizeLimit:4096000];
  dict[@"image"] = [imageData base64EncodedStringWithOptions:0];
  dict[@"vehicle_license_side"] = @"front";
...
- (void)detectVehicleLicenseBackFromImage:(UIImage *)image withOptions:(NSDictionary *)options successHandler:(void (^)(id result))successHandler failHandler:(void (^)(NSError *err))failHandler {
  if(!image){
    if (failHandler) failHandler(ERR_EMPTY_IMAGE);
    return;
  }
  ...
  NSData *imageData = [self jpgDataWithImage:image sizeLimit:4096000];
  dict[@"image"] = [imageData base64EncodedStringWithOptions:0];
  dict[@"vehicle_license_side"] = @"back";
 ...
 * 车牌识别
 */
- (void)carNumberFromImage:(UIImage *)image withOptions:(NSDictionary *)options successHandler:(void (^)(id))successHandler failHandler:(void (^)(NSError *))failHandler
{
  if(!image){
    if (failHandler) failHandler(ERR_EMPTY_IMAGE);
    return;
  ...
  [self _apiRequestWithURL:URL_CAR
                   options:dict
            successHandler:successHandler
               failHandler:failHandler];
}
...
- (void) iOCRRecognitionFromImage: (UIImage *)image
  ...
  [self _apiRequestWithURL:URL_IOCR
                   options:dict
            successHandler:successHandler
               failHandler:failHandler];
}
...
- (void)_apiRequestWithURL:(NSString *)URL
  ...
  [_aipTokenManager getTokenWithSuccessHandler:^(NSString *token) {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@?%@", URL, [OcrService wwwFormWithDictionary:getParams]]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    [request setHTTPMethod:@"POST"];
    [request setValue:@"application/x-www-form-urlencoded" forHTTPHeaderField:@"Content-Type"];
    [request setValue:@"application/json" forHTTPHeaderField:@"Accept"];
    
    NSString *formData = [OcrService wwwFormWithDictionary:options];
    [request setHTTPBody:[formData dataUsingEncoding:NSUTF8StringEncoding]];
    
    [[_apiSession dataTaskWithRequest:request
                    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                      if (error) {
                        if (failHandler) failHandler([OcrService aipApiServerConnectErrorWithMessage:[error localizedDescription]]);
                        return;
                      }
                      NSError *serializedErr = nil;
                      id obj = [NSJSONSerialization JSONObjectWithData:data options:0 error:&serializedErr];
                      if (serializedErr) {
                        if (failHandler) failHandler([OcrService aipApiServerIllegalResponseError]);
                        return;
                      }
                      if (obj[@"error_code"]){
                        ...
                        if (failHandler) failHandler([OcrService aipErrorWithCode:[obj[@"error_code"] integerValue] andMessage:obj[@"error_msg"]]);
                        return;
                      }
...
+ (NSError *)aipApiServerConnectErrorWithMessage:(NSString *)message {
  ...
  return [OcrService aipErrorWithCode:283504 andMessage:err];
}

+ (NSError *)aipApiServerIllegalResponseError {
  return [OcrService aipErrorWithCode:283505 andMessage:@"Internal error. Api server responsing illegal data"];
}


+ (instancetype)shardService {
  static OcrService *sharedService = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    sharedService = [[self alloc] init];
  });
  ERR_EMPTY_IMAGE = [OcrService aipErrorWithCode:283507 andMessage:@"Empty image. 图片为空"];
