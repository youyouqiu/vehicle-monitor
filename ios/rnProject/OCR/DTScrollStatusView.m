...
- (void)setupWithTitleArr:(NSArray *)titleArr
    ...
    [self setupWithTitleArr:titleArr
             ...
             selectTabColor:DTColor(51, 158, 255, 1)
                  lineColor:DTColor(51, 158, 255, 1)];
}
- (void)setupWithTitleArr:(NSArray *)titleArr
                     type:(ScrollTapType)type
           normalTabColor:(UIColor *)normalTabColor
...
- (void)setStatusViewWithTitleArr:(NSArray *)titleArr
    ...
    self.mainScrollView.contentSize   = CGSizeMake(kScreenWidth * titleArr.count, height+20);
    self.mainScrollView.scrollEnabled = YES;//可滚动
    self.mainScrollView.bounces = NO;//到边了就不能再拖地
    // 创建列表
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (![scrollView isKindOfClass:[UITableView class]]) {
        if (_isRefresh == NO) {
          ...
          CGRect frame = _statusView.lineView.frame;
          float origin = scrollView.contentOffset.x / _tableArr.count;
          frame.origin.x = origin;
          _statusView.lineView.frame = frame;
...
- (UIScrollView *)mainScrollView {
    if (!_mainScrollView) {
        ...
        _mainScrollView.backgroundColor = UIColorFromRGB(0xf4f7fa);
